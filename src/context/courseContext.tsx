...
interface CourseContextType {
  ...
  fetchPublicCourses: (params?: PublicFetchParams) => Promise<Course[]>;
...
// NEW: Public fetch params (no admin-only filters)
interface PublicFetchParams {
  page?: number;
  limit?: number;
  search?: string;
  departmentId?: string;
...
export const CourseProvider: React.FC<CourseProviderProps> = ({ children }) => {
  ...
  const normalizeCourseOutline = useCallback(
    (
      ...
      if (
        !courseOutline ||
        (Array.isArray(courseOutline) && courseOutline.length === 0)
      ) {
        return [];
      }
      ...
      if (
        ...
        courseOutline[0] &&
        typeof courseOutline[0] === "object" &&
        "weekId" in courseOutline[0]
      ) {
        return (courseOutline as CourseOutlineWeek[]).map((week, weekIdx) => {
          // Ensure unique weekId
          const weekId =
            week.weekId && !usedWeekIds.has(week.weekId)
              ? week.weekId
          ...
          return {
            ...
            topics: week.topics.map((topic, topicIdx) => {
              ...
              const topicId =
                topic.id && !usedTopicIds.has(topic.id)
                  ? topic.id
              ...
              return {
                ...
                subtopics: (topic.subtopics || []).map((subtopic) => {
                  ...
                  const subtopicId =
                    subtopic.id && !usedSubtopicIds.has(subtopic.id)
                      ? subtopic.id
                      : generateUniqueId(
                          `subtopic-${weekIdx + 1}-${topicIdx + 1}`,
                          usedSubtopicIds
                        );
  ...
  // NEW: Public course fetching method (no admin check required)
  const fetchPublicCourses = useCallback(
    async (params: PublicFetchParams = {}): Promise<Course[]> => {
      try {
        const queryParams = new URLSearchParams();

        // Always fetch only approved courses for public access
        ...
        // Use a new public endpoint or modify existing endpoint to handle public access
        const url = `/api/public/courses?${queryParams.toString()}`;
        console.log("course data:", url)
        const response = await fetch(url, {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        });
  ...
  const uploadCourse = useCallback(
    async (courseData: Partial<ICourse>): Promise<boolean> => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "Missing required fields: courseName, courseCode, or departmentId"
          );
        }
  ...
  // FIXED: Initialize for admin users only
  useEffect(() => {
    if (
  ...
  }, [userState, hasActiveSession, isInitialized, isAdmin, fetchCourses]);
