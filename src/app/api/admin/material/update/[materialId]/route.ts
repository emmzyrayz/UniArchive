  ...
  try {
    ...
    for (const [key, value] of Object.entries(updateData)) {
      if (allowedUpdateFields.includes(key) && value !== undefined) {
        ...
        if (JSON.stringify(material[key as keyof typeof material]) !== JSON.stringify(value)) {
    ...
    if (type === 'PDF' && updateData.pdfUrl && updateData.pdfUrl !== material.pdfUrl) {
      updateObject.pdfUrl = updateData.pdfUrl;
      changedFields.push('pdfUrl');
    }
    
    if (type === 'IMAGE' && updateData.imageUrls && 
        JSON.stringify(updateData.imageUrls) !== JSON.stringify(material.imageUrls)) {
    ...
    if (type === 'VIDEO' && updateData.videoUrl && updateData.videoUrl !== material.videoUrl) {
    ...
    if (updateData.uploaderUpid && updateData.uploaderName && updateData.uploaderRole) {
      await addEditRecord(materialId, type, {
        editedBy: updateData.uploaderUpid,
        editedByName: updateData.uploaderName,
        editedByRole: updateData.uploaderRole as "admin" | "mod" | "contributor",
        changedFields,
