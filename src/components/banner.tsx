      ...
      <div className="banner-section h-[50vh] lg:h-[70vh] w-full relative overflow-hidden rounded-[14px]">
        {bannerData.map((slide, index) => (
          ...
          >
            ...
            {/* Gradient Overlay */}
            <div className="absolute inset-0 bg-gradient-to-tr from-black/90 to-white/10 z-10 rounded-[14px]"></div>

            {/* Content */}
            <div className="flex flex-col gap-3 items-center justify-center relative z-20 text-white max-w-4xl mx-auto rounded-[14px]">
              ...
              <a
                href={slide.btnUrl}
                className="flex w-fit h-[50px] items-center justify-center bg-white text-black px-6 py-2 rounded-[10px] font-semibold hover:bg-black hover:text-white transition-colors"
              >
                {slide.btnText}
              </a>
            </div>
        ...
        {/* Navigation Dots */}
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-30 gap-2">
          {bannerData.map((_, index) => (
            <button
              key={index}
              className={`w-3 h-3 rounded-full transition-colors hover:bg-white/40 ${
                index === currentSlide ? "bg-white" : "bg-gray-500/60"
              }`}
...
export const DemoCardCarousel = () => {
  ...
  const calculateCardPosition = (index: number) => {
    ...
    const baseRadius = windowWidth >= 1024 ? 700 : 550;
    const angleStep = (2 * Math.PI) / totalCards;
    const currentAngle = (index - currentIndex) * angleStep;

    // Adjust visible cards based on screen size
    const visibleRange = windowWidth >= 1024 ? 3 : 0.5; // Show 5 cards (2 on each side) on large screens, 3 cards on smaller screens
    ...
    const x =
      windowWidth >= 1024
        ...
        : (Math.sin(currentAngle) * baseRadius) / 1.5;
    const z =
      windowWidth >= 1024
        ? (Math.cos(currentAngle) * baseRadius) / 1.2
    ...
    if (isVisible) {
      ...
      const normalizedDistance =
        windowWidth >= 1024
          ...
          : 1 - (visibleRange + 1) / distanceFromCenter * 1.8;
      scale =  0.5 + normalizedDistance * 1; // Scale range: 0.6 to 1.0
      opacity = 0.4 + normalizedDistance * 1; // Opacity range: 0.4 to 1.0
    }
  ...
  return (
    <div
      className="w-full h-[350px] py-[40px] mt-[20px] flex items-center justify-center"
